...
{
    return match socksv5::read_version(&mut reader).await? {
        socksv5::SocksVersion::V5 => {
            ...
            {
                return Err(anyhow::anyhow!("this proxy only supports null authentication"));
            }
...
async fn server() -> std::io::Result<()> {
    let listener = TcpListener::bind("127.0.0.1:8964").await?;
